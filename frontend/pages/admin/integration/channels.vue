          ...
          <va-card outlined class="mb-4">
            ...
            <va-card-content>
              <div class="text-xs text-gray-400 mb-2">
                * {{ $t('integration.channels.mapping_desc') }}
              </div>
...
const mappingColumnDefs = computed(() => [
  ...
  { 
    field: 'selectedField', 
    headerName: t('integration.channels.domain_field'), 
    flex: 1, 
    minWidth: 160,
    editable: true,
...
const serializeUiData = () => {
  ...
  if (uiMappings.value.length > 0) {
    ...
    formData.value.mappingConfigJson = JSON.stringify({ mappings: mappingsToSave })
  } else {
...
const deserializeUiData = (row) => {
  uiConfig.value = { wsUrl: '', wsMethod: 'POST', wsHeaders: [], jdbcUrl: '', jdbcUser: '', jdbcPassword: '', jdbcTable: '', mqBroker: '', mqTopic: '' }
  uiMappings.value = []
  ...
  try {
    ...
    if (mapping && mapping.mappings && Array.isArray(mapping.mappings)) {
      uiMappings.value = mapping.mappings.map(m => {
        ...
        const match = m.sourceExpression?.match(/payload\['(.+)'\]/)
